...
LocalPlayer():PrintMessage(HUD_PRINTTALK, ArmuriePoliceLang[AP_Lang].APOneLifeTexte )
return
else
...
if (table.HasValue(AP_Job , team.GetName(LocalPlayer():Team())))then
    ...
    lignelabel:SetText(ArmuriePoliceLang[AP_Lang].APLabel)
...
Scroll.Button[k]:SetText(ArmuriePoliceLang[AP_Lang].APButton)
...
if LocalPlayer().DarkRPVars.money < Armes_Shop[k].Price then
			Scroll.Button[k]:SetText(ArmuriePoliceLang[AP_Lang].APMoney)
...
Scroll.Button[k].DoClick = function()
 if Armes_Shop[k].VIP == true then
	...
	LocalPlayer():PrintMessage(HUD_PRINTTALK, ArmuriePoliceLang[AP_Lang].APVIPTexte )
...
Scroll.Text[k]:SetText( ArmuriePoliceLang[AP_Lang].APTexte.." "..Armes_Shop[k].Name)
Scroll.Text[k]:SizeToContents()


Scroll.Text[k] = vgui.Create( "DLabel", Scroll )
Scroll.Text[k]:SetPos( 120, 40+sautdeligne*sautelignetext)
Scroll.Text[k]:SetText( ArmuriePoliceLang[AP_Lang].APDesc.." "..Armes_Shop[k].Desc)
Scroll.Text[k]:SizeToContents()

Scroll.Prix[k] = vgui.Create( "DLabel", Scroll )
Scroll.Prix[k]:SetPos( 120, 60+sautdeligne*sautelignetext)
Scroll.Prix[k]:SetText( ArmuriePoliceLang[AP_Lang].APWeaponMoney.." "..Armes_Shop[k].Price .." ".. ArmuriePoliceLang[AP_Lang].APGive3 )
...
Scroll.VIP[k]:SetText( ArmuriePoliceLang[AP_Lang].APVIP .." "..ArmuriePoliceLang[AP_Lang].APYes  )

else

Scroll.VIP[k]:SetText( ArmuriePoliceLang[AP_Lang].APVIP .." "..ArmuriePoliceLang[AP_Lang].APNo )
...
LocalPlayer():PrintMessage(HUD_PRINTTALK, ArmuriePoliceLang[AP_Lang].APNoTeam )	 
end
end )
...
function ENT:Draw()
	...
	for _, ent in pairs (ents.FindByClass("armurie_police")) do
    ...
    local title = ArmuriePoliceLang[AP_Lang].APTitle
